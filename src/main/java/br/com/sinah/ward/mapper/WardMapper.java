...
public class WardMapper {
    public static WardModel toModel(WardRequestDTO requestDTO){
        return new WardModel(null,null,requestDTO.name(),requestDTO.description(), LocalDateTime.now(),LocalDateTime.now());
    }
    public static WardResponseDTO toDTO(WardModel model){
        return new WardResponseDTO(model.getUuid(),model.getName(),model.getDescription(),LocalDateTime.now(),LocalDateTime.now());
    }
